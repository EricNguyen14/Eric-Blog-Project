...
function ArticleRelated({ post = null }) {
  if (!post) {
    return <></>;
  }
  const { title, authorName, slug, date } = post;
  const timePublish = date.split("T");
  let newTime = "   " + dayjs(timePublish).fromNow();

  return (
    <article className="related-post__card">
      <h2 className="related-post__title">
